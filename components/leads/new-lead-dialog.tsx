...
function LeadForm({ initialDraft, onClose, onCreate }: { initialDraft?: Partial<LeadDraft>; onClose: () => void; onCreate: (draft: LeadDraft) => Promise<void> }) {
  ...
  const submit = async (event: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (!form.company.trim()) { setError('Informe o nome da empresa.'); return; }
    if (form.email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) { setError('Informe um email válido, como nome@empresa.com, ou deixe o campo vazio.'); return; }
    if (form.value && Number(form.value) < 0) { setError('O valor estimado não pode ser negativo.'); return; }
    ...
    <DialogContent className="max-h-[92vh] overflow-y-auto rounded-[15px] border-[var(--line)] bg-[var(--surface-raised)] shadow-2xl sm:max-w-[620px]">
      <form onSubmit={submit} noValidate>
        ...
        <div className="grid gap-4 py-5 sm:grid-cols-2">
          ...
          <Field label="Email"><Input className="premium-input" type="email" value={form.email} onChange={(event) => { set('email', event.target.value); if (error) setError(''); }} placeholder="nome@empresa.com" aria-invalid={Boolean(error && error.toLowerCase().includes('email'))} /></Field>
        ...
        {error && <p role="alert" className="mb-4 rounded-md border border-red-300/50 bg-red-50 px-3 py-2.5 text-sm text-red-700 dark:bg-red-950/30 dark:text-red-200">{error}</p>}
        <DialogFooter className="border-t border-[var(--line-soft)] pt-4">
          ...
          <Button type="submit" disabled={pending} className="bg-[var(--brand-deep)] text-white hover:opacity-90 dark:text-[#13231f]">{pending ? 'Salvando...' : 'Adicionar lead'}</Button>
        </DialogFooter>
      </form>
    </DialogContent>
  );
